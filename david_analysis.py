...
def read_file(file_path):
    ...
    df=pd.read_csv(file_path, usecols=[0], delimiter='\t', index_col=False, names=['UNIPROT_ACCESSION'])
    input_ids = ",".join(df['UNIPROT_ACCESSION'].astype(str).unique().tolist())
    return input_ids
    
def get_chart_report(input_file,output_file,auth_email,identifier,p_value,count,category):
    ...
    input_ids = read_file(input_file)
    ...
    with open(output_file, 'w') as w:
        w.write('Category\tTerm\tCount\t%\tPvalue\tGenes\tList Total\tPop Hits\tPop Total\tFold Enrichment\tBonferroni\tBenjamini\tFDR\n')
...
def cli_model(args):
    ...
    parser.add_argument("--output_file",default="chartReport.csv", help="输出文件名称, 默认chartReport.csv")
    parser.add_argument("--identifier",default="UNIPROT_ACCESSION",help="输入文件中数据的类型, 默认UNIPROT_ACCESSION")
    parser.add_argument("--pvalue",default=0.1,help="设定pvalue值, 默认0.1")
    parser.add_argument("--count",default=2,help="设定最少数量阈值, 默认2")
    parser.add_argument("--category",default='',help="设定需要过滤的category值, 默认不过滤")
